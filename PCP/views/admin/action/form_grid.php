...
<fieldset class="ui-helper-reset ui-widget-content ui-corner-all">
	...
	<form method="post" action="<?php echo($grid_action_form_action); ?>">
		...
		<?php if (isset($event->grid_action_id)){ ?><input type="hidden" name="grid_action_id" value="<?php echo($event->grid_action_id); ?>" /> <?php } ?>
		...
		<select name="location_select" >
			...
			{
				$selected = '';
				if ($event->action_value == $location->id) $selected = ' selected="selected" ';
		...
		<label id="action_value" for="action_value">Event Value:
			<textarea rows="5" cols="70" name="action_value"><?php echo($event->action_value); ?></textarea>
		</label>
