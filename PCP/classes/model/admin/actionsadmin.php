...
{		
	...
	/*
		Searches the Action directory for class files 
	*/
	static function loadActionDefs($action_type=ACTION_TYPE_NULL)
	{	
		self::cacheJSActionDefs(); // cache JS action files
		$dir = 'classes/action/';
		return self::searchFilesforActionDefs(APPPATH.$dir);		
	}
	
	// recursive search for actiondef class files
	private static function searchFilesforActionDefs($dir='')
	{
		$actiontypes = array(); // array to hold any action classes we find
		$files = scandir($dir);// get all the files in the action directory
		...
		foreach($files as $file)
		{
			...
			{
				$actiontypes = array_merge($actiontypes,self::searchFilesforActions($dir.$file.'/'));
			}
		}
		return $actiontypes;
	}
	...
	*/ 
	...
	static function loadActionTypeActions($action_type=ACTION_TYPE_NULL)
	{
		...
		if ($action_type != ACTION_TYPE_NULL)
		{
			...
			{
				if (count(array_intersect($actionDef->getAllowedTypes(),array($action_type))) > 0)
				{
