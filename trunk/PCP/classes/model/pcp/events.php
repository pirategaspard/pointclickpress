...
{	
	...
	{		
		...
		if (isset($args['event_type']))
		{
			...
			{	
				...
				break;
				default:
					$event = new Model_Base_Event($args);
				break;
			}
		}
		else
	...
	static function getSceneItemEvents($item_locations)
	{
		...
		if (isset($item_locations['griditems']))
		{
			...
			{
				$events = array_merge(self::getItemDefEvents(array('itemdef_id'=>$griditem['itemdef_id'])),$events);
				$events = array_merge(self::getItemStateEvents(array('itemstate_id'=>$griditem['itemstate_id'])),$events);
			}
		}
		return $events;
	}
	...
	/*
         ...
         */
        static function getCellEvents($args=array())
        {
                $events = array();
                if ((isset($args['scene_id']))&&(isset($args['cell_id'])))
		{
			$q = '  SELECT  e.id,
	                ...
	                if (count($events_temp) > 0)
        	        {
                        	...
                        	{
                                	$events[] = self::getGridEvent()->init($event_temp); 
	                        }
	                }
		}
                return $events;
...
/*	static function getGridItemEvents($griditem_id=0,$scene_id=0)
	{
		...
		{
			$events = self::getGridItemEvents(array('griditem_id'=>$griditem_id,'scene_id'=>$scene_id));
		}
		return $events;
	}
*/
    ...
    */
	static function doEvents($events)
	{
		...
		foreach($events as $event)
		{
			...
			if ($event_obj instanceof interfaces_iPCPevent)
			{
    ...
    */
	...
	{	
		return Cache::instance()->get('js_event_defs',Model_Admin_EventsAdmin::cacheJSEventDefs());
	}
