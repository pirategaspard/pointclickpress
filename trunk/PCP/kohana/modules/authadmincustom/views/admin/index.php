...
//   $row['last_failed_login'] = Helper_Format::relative_time(strtotime($row['last_failed_login']));
   // add actions
   $row['actions'] = Html::anchor('admin/user/edit/'.$row['id'], __('Edit')).' | '.Html::anchor('admin/user/delete/'.$row['id'], __('Delete'));
   // set roles
   $row['role'] = '';
   ...
   if(!empty($providers)) {
      ...
      if($identities->count() > 0) {
         foreach($identities as $identity) {
            $row['identities'] .= '<img src="'.Kohana::$base_url.'/images/tiny/'.$identity->provider.'.png"> ';
         }
      }
   }
   $data[] = $row;
